...
class Overlord:
    ...
    def choose_drone(self, zerg_type):
        if zerg_type is "SCOUT":
            for key, value in self.zerg.items():
                print("This is key of zerg.items ", key)
                print("This is value type of zerg.items ", type(value))
    ...
    def draw_map(self):
        print("This is self.option coming up for deployment")
        print(self.option)
        my_map = self.map_object.map_symbols
        print("?" * 100)
        print("pre-deployment map")
        print(self.zerg_to_map[self.zerg_id])
        map_to_draw = self.zerg_to_map[self.zerg_id]
        print("THis is map_to_draw")
        print()
        print("This is map_to_draw answer -> ", map_to_draw)
        print()
        print("------------------------------------------------------------------------------------------------")
        print("This is map of zerg scout")
        for key in my_map:
            map_to_draw = key    
            if my_map.get(map_to_draw, "None"):
                #print(my_map[map_to_draw])
                print("This is map for -> ", map_to_draw)
                self.min_x = min(my_map[map_to_draw])[0]
                self.max_x = max(my_map[map_to_draw])[0]
                self.min_y = min(my_map[map_to_draw], key=lambda map_list: map_list[1])[1]
                self.max_y = max(my_map[map_to_draw], key=lambda map_list: map_list[1])[1]
                for x in range(self.min_x, self.max_x + 1):
                    for y in range(self.min_y, self.max_y + 1):
                        symbol = (x, y)
                        print(my_map[map_to_draw].get(symbol, "?"), end=" ")
                    print()
            else:
                print("Not yet")
    ...
    def action(self, context):
        ...
        print()
        if act == 0:
            statement =  'RETURN {}'.format(choice(list(self.zerg.keys())))
            ...
            if self.zerg_id in self.zerg_to_map:
                if self.zerg_status[self.zerg_id] == "D":
                     ...
                     statement = 'DEPLOY {} {}'.format(self.zerg_id, self.option)
            else:
                self.zerg_to_map[self.zerg_id] = self.option
                self.zerg_status[self.zerg_id] = "D"
                #return 'DEPLOY {} {}'.format(self.zerg_id, self.option)
                statement = 'DEPLOY {} {}'.format(self.zerg_id, self.option)
        print("before self option printed in overlord class")
        print()
        self.draw_map()
        return statement
